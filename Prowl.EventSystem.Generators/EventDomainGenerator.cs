...
{
    private const string EventDomainAttrFqn = "Prowl.EventSystem.EventDomainAttribute";
    private const string EventArgsAttrFqn = "Prowl.EventSystem.EventArgsAttribute";
    private const string EventKeyFqn = "global::Prowl.EventSystem.EventKey";
    private const string UnitFqn = "global::Prowl.EventSystem.Unit";
    ...
    private static EventDomainInfo? GetDomainInfo(GeneratorAttributeSyntaxContext ctx, CancellationToken ct)
    {
        ...
        {
            if (attrData.AttributeClass?.ToDisplayString() == "Prowl.EventSystem.EventDomainAttribute")
            {
        ...
        foreach (var member in classSymbol.GetMembers())
        {
                ...
                && field.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) == EventKeyFqn)
            {
                ...
                {
                    if (attr.AttributeClass?.ToDisplayString() == "Prowl.EventSystem.EventArgsAttribute"
    ...
    private static void Execute(SourceProductionContext spc, EventDomainInfo domain)
    {
        ...
        sb.AppendLine("// Generated by Prowl.EventSystem.Generators â€” do not edit.");
        ...
        {
            sb.AppendLine($"{ci}    [global::Prowl.EventSystem.EventArgs(typeof({evt.ArgsTypeFqn}))]");
        ...
        sb.AppendLine($"{ci}{fieldDecl} global::Prowl.EventSystem.EventManager<EventTypes> {managerField} = new({globalArg});");
        sb.AppendLine();
        sb.AppendLine($"{ci}/// <summary>Gets the <see cref=\"global::Prowl.EventSystem.EventManager{{T}}\"/> for this event domain. For instance domains, dispose this when the owner is no longer needed.</summary>");
        sb.AppendLine($"{ci}public{memberStatic} global::Prowl.EventSystem.EventManager<EventTypes> Manager => {managerField};");
        sb.AppendLine();
        ...
        foreach (var evt in domain.Events)
        {
            ...
            if (evt.IsUnit)
            {
                ...
                sb.AppendLine($"{ci}public{memberStatic} global::Prowl.EventSystem.EventAccessor<EventTypes> {evt.Name}");
                sb.AppendLine($"{ci}{{");
            ...
            else
            {
                ...
                sb.AppendLine($"{ci}public{memberStatic} global::Prowl.EventSystem.EventAccessor<EventTypes, {argsType}> {evt.Name}");
                sb.AppendLine($"{ci}{{");
                sb.AppendLine($"{ci}    get => new({managerField}, EventTypes.{evt.Name});");
                sb.AppendLine($"{ci}    set {{ }}");
                sb.AppendLine($"{ci}}}");
        ...
        {
            string argsType = evt.ArgsTypeFqn;
            string containerType = $"global::Prowl.EventSystem.EventDelegateContainer<EventTypes, {argsType}>";
    ...
    private static void EmitUnitMethods(StringBuilder sb, string ci, string name, string containerType, string memberStatic, string managerField)
    {
        ...
        // GlobalInvoke (parameterless) â€” always static
        ...
        sb.AppendLine($"{ci}    => global::Prowl.EventSystem.EventManager<EventTypes>.GlobalInvokeEvent(EventTypes.{name});");
        sb.AppendLine();

        // GlobalInvokeAsync (parameterless) â€” always static
        ...
        sb.AppendLine($"{ci}    => global::Prowl.EventSystem.EventManager<EventTypes>.GlobalInvokeEventAsync(EventTypes.{name});");
        sb.AppendLine();
        ...
        string asyncContainerType = $"global::Prowl.EventSystem.AsyncEventDelegateContainer<EventTypes, {UnitFqn}>";
    ...
    private static void EmitTypedMethods(StringBuilder sb, string ci, string name, string argsType, string containerType, string memberStatic, string managerField)
    {
        ...
        // GlobalInvoke (typed) â€” always static
        sb.AppendLine($"{ci}/// <summary>Invokes <see cref=\"EventTypes.{name}\"/> across all global managers of this domain.</summary>");
        sb.AppendLine($"{ci}[global::System.Runtime.CompilerServices.MethodImpl(global::System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"{ci}public static void GlobalInvoke{name}({argsType} args)");
        sb.AppendLine($"{ci}    => global::Prowl.EventSystem.EventManager<EventTypes>.GlobalInvokeEvent(EventTypes.{name}, args);");
        sb.AppendLine();

        // GlobalInvokeAsync (typed) â€” always static
        sb.AppendLine($"{ci}/// <summary>Asynchronously invokes <see cref=\"EventTypes.{name}\"/> across all global managers of this domain, awaiting async handlers.</summary>");
        sb.AppendLine($"{ci}public static global::System.Threading.Tasks.Task GlobalInvoke{name}Async({argsType} args)");
        sb.AppendLine($"{ci}    => global::Prowl.EventSystem.EventManager<EventTypes>.GlobalInvokeEventAsync(EventTypes.{name}, args);");
        sb.AppendLine();
        ...
        string asyncContainerType = $"global::Prowl.EventSystem.AsyncEventDelegateContainer<EventTypes, {argsType}>";
