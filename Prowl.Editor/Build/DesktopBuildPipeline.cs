...
public sealed class DesktopBuildPipeline : IBuildPipeline
{
        ...
        CancellationToken cancellation = default)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(defines))
                args.Append($" -p:DefineConstants=\"{defines.Replace(";", "%3B")}\"");
    ...
    internal static string BuildDefineString(BuildSettings settings, BuildTarget target)
    {
        ...
        // Enable profiling instrumentation in debug builds so the editor
        // can connect to the player and display remote profiling data.
        string config = settings.Configuration ?? "Release";
        if (string.Equals(config, "Debug", StringComparison.OrdinalIgnoreCase))
        ...
        Runtime.Graphite.GraphicsBackendType renderingBackend = Runtime.Graphite.GraphicsBackendType.OpenGL)
    {
        ...
        // In debug builds, start the profiler server so the editor can connect
        // remotely and display live profiling data from the running player.
        string profilerStart = isDebug
            ? """
                            // Start the remote profiler server for editor connection.
                            Prowl.Runtime.Profiling.ProfilerServer.Start();
              """
            : "";
        string profilerStop = isDebug
            ? """
                                Prowl.Runtime.Profiling.ProfilerServer.Stop();
              """
            : "";
