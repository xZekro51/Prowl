...
[assembly: AssemblyInformationalVersion("1.0.10+ff99147ce244d5d50fe3074c12675cef971dfba1")]
...
public class Event<T> where T : struct, Enum
{
	...
	public void Invoke<TArgs>(TArgs args)
	{
		...
		lock (_lock)
		{
			array = (_typedSnapshots.TryGetValue(typeof(TArgs), out object value) ? ((EventDelegateContainer<T, TArgs>[])value) : Array.Empty<EventDelegateContainer<T, TArgs>>());
		}
		Span<EventDelegateContainer<T, TArgs>> span = array.AsSpan();
	...
	public async Task InvokeAsync<TArgs>(TArgs args)
	{
		...
		lock (_lock)
		{
			typedSnapshot = (_typedSnapshots.TryGetValue(typeof(TArgs), out object value) ? ((EventDelegateContainer<T, TArgs>[])value) : Array.Empty<EventDelegateContainer<T, TArgs>>());
		}
		for (int j = 0; j < typedSnapshot.Length; j++)
		{
			if (typedSnapshot[j] is IAsyncInvocable<TArgs> asyncInvocable)
			{
				await asyncInvocable.InvokeAsync(args).ConfigureAwait(continueOnCapturedContext: false);
			}
			else
			{
	...
	public ReadOnlySpan<EventDelegateContainer<T, TArgs>> GetHandlers<TArgs>()
	{
		lock (_lock)
		{
			...
			{
				return (EventDelegateContainer<T, TArgs>[])value;
			}
	...
	private void RebuildSnapshot()
	{
		...
		EventDelegateContainer<T>[] array = new EventDelegateContainer<T>[num];
		int num2 = 0;
	...
	private void RebuildTypedSnapshots()
	{
		...
		for (int i = 0; i < _sortedKeys.Count; i++)
		{
			...
			for (int j = 0; j < list.Count; j++)
			{
				...
				if (!dictionary.TryGetValue(argsType, out var value))
				{
					value = (dictionary[argsType] = new List<EventDelegateContainer<T>>());
				}
				value.Add(eventDelegateContainer);
			}
		}
		if (dictionary == null)
		{
		...
		{
			if (!s_arrayBuilders.TryGetValue(item.Key, out Func<List<EventDelegateContainer<T>>, object> value2))
			{
				value2 = CreateArrayBuilder(item.Key);
				s_arrayBuilders[item.Key] = value2;
			}
			_typedSnapshots[item.Key] = value2(item.Value);
		}
	}
	...
	{
		EventDelegateContainer<T, TArgs>[] array = new EventDelegateContainer<T, TArgs>[list.Count];
	...
	{
		MethodInfo method = typeof(Event<T>).GetMethod("BuildTypedArray", BindingFlags.Static | BindingFlags.NonPublic).MakeGenericMethod(argsType);
		return (Func<List<EventDelegateContainer<T>>, object>)Delegate.CreateDelegate(typeof(Func<List<EventDelegateContainer<T>>, object>), method);
	}
}
/// <summary>
...
public readonly struct EventAccessor<TEnum>(EventManager<TEnum> manager, TEnum eventType) where TEnum : struct, Enum
{
	...
	{
		accessor._manager.AddNewDelegate(accessor._eventType, handler);
		return accessor;
	}
	...
	{
		accessor._manager.AddNewAsyncDelegate(accessor._eventType, handler);
		return accessor;
	}
...
public abstract class EventDelegateContainer<T> : IEventDelegateContainer, IDisposable where T : struct, Enum
{
	...
	public string SourceDescription
	{
		get
		{
			...
			return $"{SourceFile}:{SourceLine} ({SourceMember})";
		}
	}
...
public class EventManager<T> : IDisposable where T : struct, Enum
{
	...
	public async Task InvokeEventAsync<TArgs>(T eventType, TArgs args)
	{
		if (!_disposed && Enabled)
		{
			...
			{
				await value.InvokeAsync(args).ConfigureAwait(continueOnCapturedContext: false);
			}
		}
	}
	...
	/// </summary>
	public EventDelegateContainer<T, TArgs> AddNewDelegate<TArgs>(T eventType, Action<TArgs> eventDelegate, EventPriority priority = default(EventPriority), string? sourceFile = null, int sourceLine = 0, string? sourceMember = null)
	{
	...
	/// </summary>
	public EventDelegateContainer<T, Unit> AddNewDelegate(T eventType, Action eventDelegate, EventPriority priority = default(EventPriority), string? sourceFile = null, int sourceLine = 0, string? sourceMember = null)
	{
		ObjectDisposedException.ThrowIf(_disposed, this);
	...
	/// </summary>
	public AsyncEventDelegateContainer<T, TArgs> AddNewAsyncDelegate<TArgs>(T eventType, Func<TArgs, Task> eventDelegate, EventPriority priority = default(EventPriority), string? sourceFile = null, int sourceLine = 0, string? sourceMember = null, bool allowMultiple = false)
	{
		ObjectDisposedException.ThrowIf(_disposed, this);
		if (!EventArgsContract<T>.IsValid<TArgs>(eventType))
		{
	...
	/// </summary>
	public AsyncEventDelegateContainer<T, Unit> AddNewAsyncDelegate(T eventType, Func<Task> eventDelegate, EventPriority priority = default(EventPriority), string? sourceFile = null, int sourceLine = 0, string? sourceMember = null, bool allowMultiple = false)
	{
		ObjectDisposedException.ThrowIf(_disposed, this);
		if (!EventArgsContract<T>.IsValid<Unit>(eventType))
		{
	...
	public static async Task GlobalInvokeEventAsync<TArgs>(T eventType, TArgs args)
	{
		...
		foreach (EventManager<T> eventManager in snapshot)
		{
			if (eventManager.Enabled)
			{
				await eventManager.InvokeEventAsync(eventType, args).ConfigureAwait(continueOnCapturedContext: false);
			}
		}
	}
